...
const nextConfig: NextConfig = {
   ...
   *
   * 처음에는 "192.168.0.*" 와 "192.168.1.*" 둘만 적었는데, 정작 이 PC 가
   * 받은 주소는 192.168.35.215 였다. 대역을 통째로 적겠다는 뜻은 옳았지만
   * 두 개만 적어서 결국 빠뜨린 것이다. 그래서 셋째 칸도 와일드카드로 연다.
   *
   * 칸마다 별표 하나씩 네 칸(192.168.*.*)이어야 한다. Next 의 대조기는 칸을
   * 뒤에서부터 하나씩 맞추고 마지막에 남은 칸이 없어야 통과시키므로
   * (node_modules/next/dist/server/app-render/csrf-protection.js),
   * "192.168.*" 나 "192.168.**" 는 칸 수가 모자라 오히려 막힌다 — 실측으로
   * 확인했다. 줄여 쓰고 싶어지더라도 그대로 둘 것.
