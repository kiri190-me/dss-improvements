 ...
 *
 *   https://improvements.dss21.co.kr/api/auth/sso/start?returnTo=//가짜.example
 *
 ...
 *
 * ── 이웃 시스템과의 관계 ────────────────────────────────────────────────
 * 이 판정은 원래 이 저장소의 auth/guards.ts 안에 **네 줄**로 있었다('/' 로
 * 시작하는가 · '//' 로 시작하지 않는가 · 역슬래시가 없는가). 내자(dss-po)가
 * 그것을 본떠 가면서 네 가지를 더했고, 2026-09-21 에 그 판을 여기로 되가져온
 * 것이 이 파일이다. 더해진 넷은 이렇다:
 *   1. 한 겹 푼 형태까지 검사 — "/%2F%2F가짜.example" 을 막는다
 *   2. 길이 상한 — 왕복 쿠키가 조용히 저장되지 않는 것을 막는다(아래)
 *   3. 로그인 통로 자기 참조 거절 — 무한 되돌기를 막는다
 *   4. 머리말에 실을 수 있는 형태로의 정규화 — 한글 주소의 500 을 막는다(아래)
 * 🔴 짝이 되는 파일은 dss-po/src/lib/auth/return-to.ts 다. 두 저장소가 서로를
 * 참조하지 않아 공유 패키지가 없다 — **한쪽을 고치면 나머지도 봐야 한다.**
 *
 * A/S(RF_Service_System)에는 **같은 처리가 없다** — 그쪽 로그인은 돌아갈 주소를
 * 아예 나르지 않고 역할에 따라 정해진 곳으로만 간다
 ...
 *
 * 이 값은 improvements_sso_tx 쿠키에 **통째로 담겨** 포털을 다녀온다. 쿠키
 * 하나의 한도는 브라우저마다 대략 4096바이트고, 넘으면 저장 자체가 조용히
 * 실패해 로그인이 이유 없이 「만료」로 끝난다.
 *
 ...
 * 두 겹, 세 겹까지 풀면 "/?q=50%25" 같은 멀쩡한 주소를 거절하게 된다.
 */
